...
public class ManageWorkController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ManageWorkController</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ManageWorkController at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
...
//            request.getRequestDispatcher("worklist.jsp").forward(request, response);
            if(loginuid==null || loginuid.isEmpty()){
                ...
                if(adminlist.contains(loginuid)){
                    ...
                    request.setAttribute("uid", uid);
                    request.getRequestDispatcher("WorkList?per=false").forward(request, response);
                }
            }            
