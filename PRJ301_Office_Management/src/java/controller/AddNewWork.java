...
public class AddNewWork extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String workrequest =request.getParameter("request");
        String worksubmit=request.getParameter("submit");
        Date workstartdate=Date.valueOf(request.getParameter("date"));
        String workstatus=request.getParameter("status");
        Work w=new Work(workid, worktitle, workrequest, worksubmit, workstartdate, workstatus);
        try {
            wdb.addNewWork(w);
